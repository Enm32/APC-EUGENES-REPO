...
public ResponseEntity<signUpresponse> add_student(studentSignup sSu) {
   ...
   students newStudents=students.builder().student_name(sSu.getStudent_name()).parent_name(sSu.getParent_name()).parent_phone(sSu.getParentPhone()).studentGrade(s_grade).student_name(sSu.getStudent_name()).emailAddress(sSu.getEmailAddress()).build();
